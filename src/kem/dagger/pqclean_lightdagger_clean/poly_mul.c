...
#define N_RES (DAGGER_N << 1)
#define N_SB (DAGGER_N >> 2)
...
/* res += a*b */
void PQCLEAN_LIGHTDAGGER_CLEAN_poly_mul(poly *c, const poly *a, const poly *b, const int accumulate) {
    uint16_t C[2 * DAGGER_N] = {0};
    size_t i;
    ...
    /* reduction */
    if (accumulate == 0) {
        for (i = DAGGER_N; i < 2 * DAGGER_N; i++) {
            c->coeffs[i - DAGGER_N] = (C[i - DAGGER_N] - C[i]);
        }
    } else {
        for (i = DAGGER_N; i < 2 * DAGGER_N; i++) {
            c->coeffs[i - DAGGER_N] += (C[i - DAGGER_N] - C[i]);
