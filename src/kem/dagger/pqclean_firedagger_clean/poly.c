...
void PQCLEAN_FIREDAGGER_CLEAN_MatrixVectorMul(poly c[DAGGER_L], const poly A[DAGGER_L][DAGGER_L], const poly s[DAGGER_L], int16_t transpose) {
    ...
    if (transpose) {
        for (i = 0; i < DAGGER_L; i++) {
            PQCLEAN_FIREDAGGER_CLEAN_poly_mul(&c[i], &A[0][i], &s[0], 0);
            for (j = 1; j < DAGGER_L; j++) {
                PQCLEAN_FIREDAGGER_CLEAN_poly_mul(&c[i], &A[j][i], &s[j], 1);
            }
        }
    } else {
        for (i = 0; i < DAGGER_L; i++) {
            PQCLEAN_FIREDAGGER_CLEAN_poly_mul(&c[i], &A[i][0], &s[0], 0);
            for (j = 1; j < DAGGER_L; j++) {
                PQCLEAN_FIREDAGGER_CLEAN_poly_mul(&c[i], &A[i][j], &s[j], 1);
            }
        }
    }
}
...
void PQCLEAN_FIREDAGGER_CLEAN_InnerProd(poly *c, const poly b[DAGGER_L], const poly s[DAGGER_L]) {
    ...
    PQCLEAN_FIREDAGGER_CLEAN_poly_mul(c, &b[0], &s[0], 0);
    for (i = 1; i < DAGGER_L; i++) {
        PQCLEAN_FIREDAGGER_CLEAN_poly_mul(c, &b[i], &s[i], 1);
    }
}
...
void PQCLEAN_FIREDAGGER_CLEAN_GenMatrix(poly A[DAGGER_L][DAGGER_L], const uint8_t seed[DAGGER_SEEDBYTES]) {
    ...
    for (i = 0; i < DAGGER_L; i++) {
        PQCLEAN_FIREDAGGER_CLEAN_BS2POLVECq(A[i], buf + i * DAGGER_POLYVECBYTES);
    }
}
...
void PQCLEAN_FIREDAGGER_CLEAN_GenSecret(poly s[DAGGER_L], const uint8_t seed[DAGGER_NOISESEEDBYTES]) {
    ...
    for (i = 0; i < DAGGER_L; i++) {
        PQCLEAN_FIREDAGGER_CLEAN_cbd(s[i].coeffs, buf + i * DAGGER_POLYCOINBYTES);
    }
}
