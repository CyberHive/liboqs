...
int PQCLEAN_LIGHTDAGGER_AVX2_crypto_kem_keypair(uint8_t *pk, uint8_t *sk) {
    size_t i;

    PQCLEAN_LIGHTDAGGER_AVX2_indcpa_kem_keypair(pk, sk); // sk[0:DAGGER_INDCPA_SECRETKEYBYTES-1] <-- sk
    ...
    // This is output when check in PQCLEAN_LIGHTDAGGER_AVX2_crypto_kem_dec() fails.
    return (0);
}

int PQCLEAN_LIGHTDAGGER_AVX2_crypto_kem_enc(uint8_t *c, uint8_t *k, const uint8_t *pk) {
    ...
    PQCLEAN_LIGHTDAGGER_AVX2_indcpa_kem_enc(c, buf, kr + 32, pk); // buf[0:31] contains message; kr[32:63] contains randomness r;
...
int PQCLEAN_LIGHTDAGGER_AVX2_crypto_kem_dec(uint8_t *k, const uint8_t *c, const uint8_t *sk) {
    size_t i;
    uint8_t fail;
    ...
    PQCLEAN_LIGHTDAGGER_AVX2_indcpa_kem_dec(buf, sk, c); // buf[0:31] <-- message
    ...
    PQCLEAN_LIGHTDAGGER_AVX2_indcpa_kem_enc(cmp, buf, kr + 32, pk);

    fail = PQCLEAN_LIGHTDAGGER_AVX2_verify(c, cmp, DAGGER_BYTES_CCA_DEC);

    sha3_256(kr + 32, c, DAGGER_BYTES_CCA_DEC); // overwrite coins in kr with h(c)

    PQCLEAN_LIGHTDAGGER_AVX2_cmov(kr, sk + DAGGER_SECRETKEYBYTES - DAGGER_KEYBYTES, DAGGER_KEYBYTES, fail);
