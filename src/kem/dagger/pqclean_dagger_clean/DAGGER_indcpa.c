...
void PQCLEAN_DAGGER_CLEAN_indcpa_kem_keypair(uint8_t pk[DAGGER_INDCPA_PUBLICKEYBYTES], uint8_t sk[DAGGER_INDCPA_SECRETKEYBYTES]) {
    ...
    PQCLEAN_DAGGER_CLEAN_GenSecret(s, rand);
    PQCLEAN_DAGGER_CLEAN_POLVECq2BS(sk, s);

    PQCLEAN_DAGGER_CLEAN_GenMatrix(A, seed_A); // sample matrix A
    PQCLEAN_DAGGER_CLEAN_MatrixVectorMul(res, (const poly (*)[DAGGER_L])A, (const poly *)s, 1); // Matrix in transposed order
    ...
    PQCLEAN_DAGGER_CLEAN_POLVECp2BS(pk, res); // pack public key
}
...
void PQCLEAN_DAGGER_CLEAN_indcpa_kem_enc(uint8_t ciphertext[DAGGER_BYTES_CCA_DEC], const uint8_t m[DAGGER_KEYBYTES], const uint8_t noiseseed[DAGGER_NOISESEEDBYTES], const uint8_t pk[DAGGER_INDCPA_PUBLICKEYBYTES]) {
    ...
    PQCLEAN_DAGGER_CLEAN_GenSecret(s, noiseseed);
    PQCLEAN_DAGGER_CLEAN_GenMatrix(A, seed_A);
    PQCLEAN_DAGGER_CLEAN_MatrixVectorMul(res, (const poly (*)[DAGGER_L])A, (const poly *)s, 0); // 0 => not transposed
    ...
    PQCLEAN_DAGGER_CLEAN_POLVECp2BS(ciphertext, res);

    // vector-vector scalar multiplication with mod p
    PQCLEAN_DAGGER_CLEAN_BS2POLVECp(temp, pk);
    PQCLEAN_DAGGER_CLEAN_InnerProd(vprime, temp, s);
    PQCLEAN_DAGGER_CLEAN_BS2POLmsg(message, m);
    ...
    PQCLEAN_DAGGER_CLEAN_POLT2BS(msk_c, vprime);
}
...
void PQCLEAN_DAGGER_CLEAN_indcpa_kem_dec(uint8_t m[DAGGER_KEYBYTES], const uint8_t sk[DAGGER_INDCPA_SECRETKEYBYTES], const uint8_t ciphertext[DAGGER_BYTES_CCA_DEC]) {
    ...
    PQCLEAN_DAGGER_CLEAN_BS2POLVECq(s, sk);
    PQCLEAN_DAGGER_CLEAN_BS2POLVECp(temp, ciphertext);
    PQCLEAN_DAGGER_CLEAN_InnerProd(&temp[0], temp, s);

    PQCLEAN_DAGGER_CLEAN_BS2POLT(cm, packed_cm);
    ...
    PQCLEAN_DAGGER_CLEAN_POLmsg2BS(m, v);
}
